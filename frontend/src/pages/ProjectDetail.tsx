...
const ProjectDetail = () => {
  ...
  const isProjectOwner = !!user && user.role === 'org' && project?.organization?.userId === user.id;
  ...
  useEffect(() => {
    if (id && project && user?.role === 'org' && project.organization?.userId === user.id) {
      loadMyUpdates();
    }
  }, [id, user, project?.organization?.userId]);
  ...
  const loadProject = async () => {
    try {
      ...
      // 公开详情只含审核通过的动态；组织本人的完整动态由 loadMyUpdates 补充
  ...
  // 组织本人查看自己项目时，拉取含待审核/已驳回（含原因）的完整动态
  const loadMyUpdates = async () => {
    if (!id) return;
    try {
      const mineRes = await projectAPI.getMyProjectUpdates(id);
      setUpdates(mineRes.data.updates);
    } catch (error) {
